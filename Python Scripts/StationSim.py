...
class Agent:
    ...
    def __init__(self, model, unique_id):
        ...
        """
        # Required
        self.unique_id = unique_id #assign id
        self.active = 0  # 0 Not Started, 1 Active, 2 Finished
        model.pop_active += 1 #increase model pop
        # Location
        self.location = model.loc_entrances[np.random.randint(model.entrances)] #assign random entrance
        self.location[1] += model.entrance_space * (np.random.uniform() - .5) # ! random spread over entrance to prevent overlap
        self.loc_desire = model.loc_exits[np.random.randint(model.exits)] #assign random exit
        # Parameters
        self.time_activate = np.random.exponential(model.entrance_speed) # ! random time until activation
        self.speed_desire = max(np.random.normal(model.speed_desire_mean, model.speed_desire_std), 2*model.speed_min) # !random desired speed truncated norm?
        self.speeds = np.arange(self.speed_desire, model.speed_min, -model.speed_step)#! a range of speeds between minimum and desired?
    ...
    def activate(self, model):
        ...
        """
        if not self.active and model.time_id > self.time_activate: # if current time> activation time
            self.active = 1
            self.time_start = model.time_id # where agent enters model
            self.time_expected = np.linalg.norm(self.location - self.loc_desire) / self.speed_desire #expected time til agent leaves
        return
    ...
    def neighbourhood(self, model, new_location, do_kd_tree=True):
        """
        check for collisions by searching for any agents within a suitable region
    ...
    def lerp(self, loc1, loc2, speed):
        """
        interpolate position between current and desired based on current speed
        loc2- old loc
        loc1 - new loc
        """
...
class Model:
    ...
    def __init__(self, params):
        ...
        pop_total': total agents
        entrances: for agents
        entrance_space: size of entrances
        entrance_speed: initial agent speed
        exits: number of exits
        exit_space: exit size
        speed_min': minimum speed
        speed_desire_mean: desired speed mean
        speed_desire_std : desired speed mean
        separation': proximity for neighbourhood?,
        batch_iterations': ?,
        do_save': bool for saving trajectory
        do_ani': bool for showing animation
        """
        self.params = (params,)
    ...
    def batch(self):
        ...
        #locs={}
        #for i in range(len(self.agents)):
        #    locs[i] = self.agents[i].history_loc
...
if __name__ == '__main__':
    ...
    # !Pool allows parallel computation of multiple agents
